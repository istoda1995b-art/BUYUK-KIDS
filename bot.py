...
async def cmd_start(message: types.Message, state: FSMContext):
    ...
    await message.answer("👋 Xush kelibsiz!\n\n🛒 Mahsulotlarimizni ko'rib buyurtma bering.\nQuyidagi tugmalardan foydalaning:", reply_markup=main_menu_keyboard(user_id))
...
async def worker_login_start(message: types.Message, state: FSMContext):
    ...
    if is_admin(user_id) or is_worker(user_id):
        ...
        return
    await message.answer("🔐 Ishchi parolini kiriting:\n(Admindan oling)", reply_markup=cancel_keyboard())
...
async def show_catalog(message: types.Message, state: FSMContext):
    ...
    builder.adjust(2, 1)
    await message.answer("📂 Kategoriyani tanlang:\n<i>(📏 — razmerlar mavjud)</i>", reply_markup=builder.as_markup(), parse_mode="HTML")
...
async def search_product_start(callback: types.CallbackQuery, state: FSMContext):
    await callback.message.answer("🔍 <b>Qidirish</b>\n\nMahsulot nomini yoki uning bir qismini yozing:", reply_markup=ReplyKeyboardMarkup(keyboard=[[KeyboardButton(text="❌ Bekor qilish")]], resize_keyboard=True), parse_mode="HTML")
...
async def ask_size_or_add(callback: types.CallbackQuery, state: FSMContext):
    ...
    if product and product.get('sizes'):
        ...
        await callback.message.answer("✅ Savatga qo'shildi! Xaridni davom ettiring yoki savatga o'ting.", reply_markup=main_menu_keyboard(callback.from_user.id))
...
async def confirm_size(callback: types.CallbackQuery, state: FSMContext):
    ...
    await callback.message.delete()
    await callback.message.answer(f"✅ <b>{product['name']}</b> [{size}] savatga qo'shildi!\nXaridni davom ettiring.", reply_markup=main_menu_keyboard(callback.from_user.id), parse_mode="HTML")
    await state.set_state(OrderStates.choosing_product)
...
async def get_name(message: types.Message, state: FSMContext):
    ...
    await message.answer("📞 Telefon raqamingizni yuboring:", reply_markup=ReplyKeyboardMarkup(keyboard=[[KeyboardButton(text="📱 Raqamni yuborish", request_contact=True)], [KeyboardButton(text="❌ Bekor qilish")]], resize_keyboard=True))
...
async def get_phone(message: types.Message, state: FSMContext):
    ...
    await message.answer("📍 <b>Manzilingizni kiriting:</b>\n\n• 📌 Geolokatsiyangizni yuboring\n• Yoki qo'lda kiriting", reply_markup=ReplyKeyboardMarkup(keyboard=[[KeyboardButton(text="📌 Joylashuvimni yuborish", request_location=True)], [KeyboardButton(text="❌ Bekor qilish")]], resize_keyboard=True), parse_mode="HTML")
...
async def get_address_location(message: types.Message, state: FSMContext):
    ...
    builder.adjust(1)
    await message.answer(f"✅ Manzil qabul qilindi:\n📍 <b>{address_text}</b>\n\n💳 To'lov usulini tanlang:", reply_markup=builder.as_markup(), parse_mode="HTML")
...
async def process_payment(callback: types.CallbackQuery, state: FSMContext):
    ...
    order_id = db.create_order(user_id=user_id, customer_name=data['customer_name'], phone=data['phone'], address=data['address'], payment=payment, items=cart_items, total=final_price, discount_pct=pct, discount_amt=discount_amt)
    if discount_amt > 0:
        disc_line = f"\n💰 Asl narx: {gross:,} so'm\n🎁 Chegirma ({pct}%): -{discount_amt:,} so'm\n✅ To'lov: <b>{final_price:,} so'm</b>"
    else:
        disc_line = f"\n💰 Jami: <b>{gross:,} so'm</b>"
    confirm_text = f"✅ <b>Buyurtmangiz qabul qilindi!</b>\n\n🔢 Buyurtma №{order_id}\n👤 {data['customer_name']}\n📞 {data['phone']}\n📍 {data['address']}\n💳 {payment}{disc_line}\n\n📦 Tez orada siz bilan bog'lanamiz!"
    await callback.message.answer(confirm_text, reply_markup=main_menu_keyboard(user_id), parse_mode="HTML")
    items_text = "\n".join([f"  • {item['name']}{' [' + item['size'] + ']' if item.get('size') else ''} ×{item['quantity']} = {item['price'] * item['quantity']:,} so'm" for item in cart_items])
    disc_admin = f"\n🎁 Chegirma ({pct}%): -{discount_amt:,} so'm" if discount_amt > 0 else ""
    admin_text = f"🆕 <b>YANGI BUYURTMA №{order_id}</b>\n\n👤 Mijoz: {data['customer_name']}\n📞 Tel: {data['phone']}\n📍 Manzil: {data['address']}\n💳 To'lov: {payment}\n\n🛒 Mahsulotlar:\n{items_text}\n\n💰 Asl narx: {gross:,} so'm{disc_admin}\n✅ <b>To'lov: {final_price:,} so'm</b>\n🆔 Telegram ID: {user_id}"
    builder = InlineKeyboardBuilder()
...
async def order_accept(callback: types.CallbackQuery):
    ...
    if order:
        try:
            await bot.send_message(order['user_id'], f"✅ Buyurtma №{order_id} qabul qilindi!\nTez orada yetkazib beramiz. Rahmat! 🙏")
        except:
            pass
...
async def order_cancel(callback: types.CallbackQuery):
    ...
    if order:
        try:
            await bot.send_message(order['user_id'], f"❌ Buyurtma №{order_id} bekor qilindi.\nQo'shimcha ma'lumot uchun biz bilan bog'laning.")
        except:
            pass
...
async def worker_panel(message: types.Message, state: FSMContext):
    if not is_worker(message.from_user.id):
        ...
        return
    await state.clear()
    await message.answer("👨‍💼 <b>Ishchi panel</b>\n\nNimani qilmoqchisiz?", reply_markup=worker_menu_keyboard(), parse_mode="HTML")
...
async def get_product_name(message: types.Message, state: FSMContext):
    ...
    if db.product_name_exists(product_name):
        ...
        builder.adjust(1)
        await message.answer(f"⚠️ <b>«{product_name}»</b> nomli mahsulot allaqachon mavjud!\n\nNima qilmoqchisiz?", reply_markup=builder.as_markup(), parse_mode="HTML")
        return
...
async def get_product_description(message: types.Message, state: FSMContext):
    ...
    if cat and cat['has_sizes']:
        await message.answer("📏 Mavjud razmerlarni kiriting (vergul bilan ajrating):\n\nMasalan: <code>86, 92, 98, 104, 110, 116</code>\n\nYoki /skip yozing", reply_markup=cancel_keyboard(), parse_mode="HTML")
        await state.set_state(AdminStates.adding_product_sizes)
    else:
        await message.answer("📸 Mahsulot rasmini yuboring (yoki /skip yozing):", reply_markup=cancel_keyboard())
...
async def get_product_sizes(message: types.Message, state: FSMContext):
    ...
    await message.answer(f"✅ Razmerlar: <b>{preview}</b>\n\n📸 Mahsulot rasmini yuboring (yoki /skip yozing):", reply_markup=cancel_keyboard(), parse_mode="HTML")
    await state.set_state(AdminStates.adding_product_photo)
...
async def get_product_photo(message: types.Message, state: FSMContext):
    ...
    db.add_product(name=data['product_name'], price=data['product_price'], description=data['product_description'], category_id=data['category_id'], photo_id=photo_id, photo_url=photo_url, sizes=sizes_str)
    size_note = ""
    if sizes_str:
        preview = " | ".join(sizes_str.split(","))
        size_note = f"\n📏 Razmerlar: {preview}"
    menu = admin_menu_keyboard() if is_admin(message.from_user.id) else worker_menu_keyboard()
    await message.answer(f"✅ '<b>{data['product_name']}</b>' qo'shildi!\n💰 Narxi: {data['product_price']:,} so'm{size_note}", reply_markup=menu, parse_mode="HTML")
    await state.clear()
...
async def edit_field_select(callback: types.CallbackQuery, state: FSMContext):
    ...
    if field == "photo":
        ...
        prompts = {"name": "🏷️ Yangi nomini kiriting:", "price": "💰 Yangi narxini kiriting (raqam):", "description": "📝 Yangi tavsifini kiriting (yoki /skip):"}
...
async def worker_orders(message: types.Message):
    ...
    if not orders:
        ...
        return
    status_map = {"pending": "⏳ Kutmoqda", "accepted": "✅ Qabul qilindi", "delivered": "🚚 Yetkazildi", "cancelled": "❌ Bekor qilindi"}
    text = "📊 <b>Oxirgi buyurtmalar:</b>\n\n"
    builder = InlineKeyboardBuilder()
    for order in orders[:20]:
        status_label = status_map.get(order['status'], "❓")
        text += f"🔢 <b>№{order['id']}</b> | {order['customer_name']} | {order['total']:,} so'm | {status_label}\n📅 {order['created_at'][:10]}\n"
...
async def worker_order_detail(callback: types.CallbackQuery):
    ...
    if not order:
        ...
        return
    status_map = {"pending": "⏳ Kutmoqda", "accepted": "✅ Qabul qilindi", "delivered": "🚚 Yetkazildi", "cancelled": "❌ Bekor qilindi"}
    items_text = "\n".join([f"  • {item['product_name']}{' [' + item['size'] + ']' if item.get('size') else ''} ×{item['quantity']} = {item['price'] * item['quantity']:,} so'm" for item in items])
    disc_line = ""
...
async def worker_order_accept(callback: types.CallbackQuery):
    ...
    if order:
        try:
            ...
            pass
    await callback.message.edit_text(callback.message.text + "\n\n✅ STATUS: QABUL QILINDI", parse_mode="HTML")
...
async def worker_order_deliver(callback: types.CallbackQuery):
    ...
    if order:
        try:
            await bot.send_message(order['user_id'], f"🚚 Buyurtma №{order_id} yetkazib berildi!\nXarid uchun rahmat! 🙏")
        except:
            pass
    await callback.message.edit_text(callback.message.text + "\n\n🚚 STATUS: YETKAZILDI", parse_mode="HTML")
...
async def worker_order_cancel(callback: types.CallbackQuery):
    ...
    if order:
        try:
            ...
            pass
    await callback.message.edit_text(callback.message.text + "\n\n❌ STATUS: BEKOR QILINDI", parse_mode="HTML")
...
async def all_orders(message: types.Message):
    ...
    for order in orders[:20]:
        ...
        text += f"{emoji} №{order['id']} | {order['customer_name']} | {order['total']:,} so'm | {order['created_at'][:10]}\n"
    await message.answer(text, parse_mode="HTML")
...
async def create_worker_pwd(callback: types.CallbackQuery):
    ...
    await callback.answer()
    await callback.message.answer(f"✅ Yangi ishchi paroli yaratildi:\n\n🔑 <code>{password}</code>\n\nBu parolni ishchiga yuboring. U /worker komandasi orqali kiradi.\n⚠️ Parol faqat bir marta ishlatiladi!", parse_mode="HTML")
...
async def fire_worker(callback: types.CallbackQuery):
    ...
    try:
        await bot.send_message(worker_tid, "ℹ️ Sizning ishchi huquqlaringiz bekor qilindi.")
    except:
        pass
